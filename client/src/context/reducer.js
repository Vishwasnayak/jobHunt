...
const reducer= (state,action) =>{
    if (action.type === DISPLAY_ALERT) {
      ...
      if (action.type === SETUP_USER_BEGIN) { /// both for register and login
        return { ...state, isLoading: true };  //return current state and flag isloading true since its new
      }
      if (action.type === SETUP_USER_SUCCESS) {
        return {
          ...state,
          isLoading: false,  //once success make it flags data is stored
      ...
      if (action.type === LOGOUT_USER) {
        return {
          ...initialState,
          user:null,  //these 4 are manually set in appcontext
          userLocation:null,
          jobLocation:null
        };
      }
      ...
      if (action.type === HANDLE_CHANGE) {
        return {
          ...state,
          isLoading: false,
          page: 1, //whenver any search, sort filters changes in jobcontainer set page=1
          [action.payload.name]:action.payload.value //return dynamiccaly access property based on name
        };
      }
